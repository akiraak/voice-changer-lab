...
- **キャラクターエディションを VCClient v.2 系にロードして Windows で配信用途に組む具体手順** → [install-vcclient-beatrice-windows.md §4.3](./install-vcclient-beatrice-windows.md#43-キャラクターエディションつくよみちゃん--刻鳴時雨--olune-の入手手順) / [§13 ライセンス・規約まわりのチェックリスト](./install-vcclient-beatrice-windows.md#13-ライセンス規約まわりのチェックリスト)
...
- **Windows でこの v1 系を w-okada/voice-changer に乗せる際の位置付け** → [install-vcclient-beatrice-windows.md §4.4](./install-vcclient-beatrice-windows.md#44-beatrice-1jvs-corpus-edition-v1-系の選択肢と営利目的禁止条項)（本書の主動線は v.2 系 + Beatrice 2。v1 系は「営利目的禁止条項」と「モデル形式非互換」の警告つきで位置付けのみ明示）。
